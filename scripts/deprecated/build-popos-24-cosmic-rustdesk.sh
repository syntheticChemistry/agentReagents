#!/bin/bash
# Build Pop!_OS 24 COSMIC + RustDesk Template
# Purpose: Create Pop!_OS 24 with COSMIC desktop + RustDesk for testing
...
VM_NAME="popos-cosmic-rustdesk-builder"
...
# RustDesk package (download to reagents)
RUSTDESK_DEB="${REAGENTS_ROOT}/packages/rustdesk-1.2.3-x86_64.deb"
mkdir -p "${REAGENTS_ROOT}/packages"

echo "╔══════════════════════════════════════════════════════════════════════╗"
echo "║                                                                      ║"
echo "║  Building Pop!_OS 24 COSMIC + RustDesk Template                     ║"
echo "║  (COSMIC Desktop + Wayland + RustDesk)                              ║"
echo "║                                                                      ║"
echo "╚══════════════════════════════════════════════════════════════════════╝"
echo ""
...
echo "✅ Using Ubuntu 24.04 cloud image as base (will add COSMIC + RustDesk)"
echo ""
...
ssh-keygen -t rsa -b 2048 -f "${SSH_KEY}" -N "" -C "popos-cosmic-rustdesk-builder" >/dev/null 2>&1
SSH_PUB_KEY=$(cat "${SSH_KEY}.pub")

# Copy RustDesk deb to a location accessible via mount
sudo mkdir -p /var/lib/libvirt/images/shared
sudo cp "${RUSTDESK_DEB}" /var/lib/libvirt/images/shared/rustdesk.deb
sudo chmod 644 /var/lib/libvirt/images/shared/rustdesk.deb

# Create cloud-init user-data (without embedding large .deb)
...
runcmd:
  ...
  - |
    ...
    DEBIAN_FRONTEND=noninteractive apt install -y -f ./rustdesk-1.2.3-x86_64.deb || true
    DEBIAN_FRONTEND=noninteractive apt install -y -f
    rm -f rustdesk-1.2.3-x86_64.deb
  
  # Configure RustDesk autostart
  - |
    mkdir -p /home/cosmic/.config/autostart
    cat > /home/cosmic/.config/autostart/rustdesk.desktop << 'RUSTDESK'
    [Desktop Entry]
    Type=Application
    Name=RustDesk
    ...
    RUSTDESK
    chown -R cosmic:cosmic /home/cosmic/.config
...
final_message: |
  COSMIC + RustDesk installation complete!
  This system will power off in 60 seconds.
EOF

# Replace SSH key placeholder
sed -i "s|SSH_PUB_KEY_PLACEHOLDER|${SSH_PUB_KEY}|g" "${USER_DATA}"
...
sudo cp "${CLOUD_INIT_ISO}" /var/lib/libvirt/images/popos-cosmic-rustdesk-cloud-init.iso
sudo chown libvirt-qemu:kvm /var/lib/libvirt/images/popos-cosmic-rustdesk-cloud-init.iso

# Create and start VM
echo ""
echo "🚀 Starting VM build process..."
echo "   This will take 15-30 minutes (COSMIC + RustDesk installation)..."
echo ""
...
sudo virt-install \
    ...
    --disk /var/lib/libvirt/images/popos-cosmic-rustdesk-cloud-init.iso,device=cdrom \
...
sudo virsh undefine "${VM_NAME}" 2>/dev/null || true
sudo rm -f /var/lib/libvirt/images/popos-cosmic-rustdesk-cloud-init.iso
...
echo "╔══════════════════════════════════════════════════════════════════════╗"
echo "║                                                                      ║"
echo "║  ✅ Pop!_OS 24 COSMIC + RustDesk Template Created Successfully!     ║"
echo "║                                                                      ║"
echo "╚══════════════════════════════════════════════════════════════════════╝"
echo ""
