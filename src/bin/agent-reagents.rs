...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Build { manifest, ssh_key, ssh_key_file } => {
...
async fn cmd_list(registry_dir: &PathBuf, verbose: bool) -> Result<()> {
    ...
    for template in templates {
        if verbose {
            ...
            println!("    Verified: {}", if template.verified { "✓" } else { "✗" });
            println!();
        } else {
...
async fn cmd_info(registry_dir: &PathBuf, name: &str) -> Result<()> {
    ...
    let info = registry.get_template(name)
    ...
    println!("Size: {} bytes ({:.2} MB)", info.size_bytes, info.size_bytes as f64 / 1_048_576.0);
...
async fn cmd_build(
    ...
    .timeout(std::time::Duration::from_secs(manifest.resources.timeout_secs));
    ...
    println!("   Size: {} bytes ({:.2} MB)", result.size_bytes, result.size_bytes as f64 / 1024.0 / 1024.0);
...
async fn cmd_delete(registry_dir: &PathBuf, name: &str, yes: bool) -> Result<()> {
    ...
    let _ = registry.get_template(name)
        .map_err(|e| anyhow::anyhow!(e))?;
